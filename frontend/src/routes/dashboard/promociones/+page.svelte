	...
	<div class="md:col-span-1 bg-white rounded-lg shadow-md p-6 h-fit">
		...
		<form onsubmit={handleCrearPromocion} class="flex flex-col gap-4">
			<div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
				<div>
					<label for="filtroCat" class="block text-sm font-semibold mb-1 text-gray-700"
				...
				<div>
					<label for="selectProd" class="block text-sm font-semibold mb-1 text-gray-700"
						>Producto a asociar:</label
					>
					<select
						id="selectProd"
			...
			<div>
				<label for="tipoPromo" class="block text-sm font-semibold mb-1 text-gray-700"
			...
			{#if tipoPromocion === 'NXM'}
				<div class="flex gap-4">
					<div class="flex-1">
						<label for="inputLleva" class="block text-sm font-semibold mb-1 text-gray-700"
							>Lleva:</label
						>
						<input
							id="inputLleva"
					...
					<div class="flex-1">
						<label for="inputPaga" class="block text-sm font-semibold mb-1 text-gray-700"
							>Paga:</label
						>
						<input
							id="inputPaga"
			...
			{:else if tipoPromocion === 'precio_fijo'}
				<div>
					<label for="inputDesc" class="block text-sm font-semibold mb-1 text-gray-700"
			...
			{:else if tipoPromocion === 'porcentaje'}
				<div>
					<label for="inputDescPorc" class="block text-sm font-semibold mb-1 text-gray-700"
						...
						{#each promociones as promo}
							<tr class="border-b border-gray-100 hover:bg-gray-50">
								<td class="py-3 px-2 font-medium">
									{productos.find((p) => p.id_producto === promo.producto_id)?.nombre ||
										promo.producto_id ||
										'Sin producto asociado'}
								</td>
								<td class="py-3 px-2">
									<span
										class="inline-block bg-primario/10 text-primario px-2 py-1 rounded text-xs font-bold"
									>
										{#if promo.tipo === 'NXM'}
