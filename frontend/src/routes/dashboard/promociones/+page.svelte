<script lang="ts">
	...
	function formatearRangoFechas(inicio: string | null | undefined, fin: string | null | undefined) {
...
{#snippet promoCard(promo: Promocion, estado: 'activa' | 'proxima' | 'expirada')}
	{@const prod =
		promo.tipo !== 'COMBO' && promo.producto_id ? obtenerProducto(promo.producto_id) : null}
	{@const nombreProd =
		promo.tipo === 'COMBO' ? generarNombresCombo(promo) : prod?.nombre || 'Producto sin nombre'}

	<div
		class="min-w-[300px] w-[300px] sm:w-[320px] flex-none snap-start bg-bg-card border border-border-color rounded-xl p-5 shadow-sm hover:border-border-color-hover flex flex-col justify-between {estado ===
		'expirada'
	...
	>
		<div>
			<div class="flex justify-between items-start mb-4">
				<div
					class="w-8 h-8 rounded {promo.tipo === 'NXM'
						? 'bg-text-primary/10 text-accent'
						: promo.tipo === 'COMBO'
							? 'bg-purple-500/15 text-purple-500'
							: 'bg-text-primary/10 text-accent'} flex items-center justify-center font-bold text-sm"
				>
					{#if promo.tipo === 'NXM'}
						<svg
							xmlns="http://www.w3.org/2000/svg"
				...
				{#if estado === 'activa'}
					<span
						class="px-3 py-1 bg-green-500/15 text-green-600 dark:text-green-400 border border-green-500/20 text-xs rounded-full font-bold"
						>Activa</span
					>
				{:else if estado === 'proxima'}
					<span
						class="px-3 py-1 bg-blue-500/15 text-blue-600 dark:text-blue-400 border border-blue-500/20 text-xs rounded-full font-bold"
						>Próxima</span
					>
				{:else}
					<span
						class="px-3 py-1 bg-text-primary/10 text-text-secondary border border-border-color text-xs rounded-full font-bold"
						>Expirada</span
					>
				{/if}
			</div>

			<h3 class="font-bold text-text-primary text-[15px] mb-1 line-clamp-1">
			...
			<div class="flex justify-between items-end mb-5">
				...
				<div
					class="bg-text-primary/5 text-text-secondary px-2 py-1 rounded text-[11px] font-semibold border border-border-color"
				>
					{#if promo.tipo === 'NXM'}
...
<div class="mb-10">
	<div
		class="flex items-center gap-2 text-green-600 dark:text-green-500 mb-4 text-xs font-bold uppercase tracking-wider"
	>
		<svg
			xmlns="http://www.w3.org/2000/svg"
			width="16"
	...
	{:else}
		<div class="relative group">
			<button
				onclick={() => moverCarrusel(carruselActivas, 'izq')}
				class="absolute -left-5 top-1/2 -translate-y-1/2 z-10 bg-bg-card border border-border-color shadow-md rounded-full p-2 text-text-secondary opacity-0 group-hover:opacity-100 hidden sm:block hover:bg-text-primary/5 focus:outline-none"
			>
				<svg
					xmlns="http://www.w3.org/2000/svg"
					width="20"
			...
			<button
				onclick={() => moverCarrusel(carruselActivas, 'der')}
				class="absolute -right-5 top-1/2 -translate-y-1/2 z-10 bg-bg-card border border-border-color shadow-md rounded-full p-2 text-text-secondary opacity-0 group-hover:opacity-100 hidden sm:block hover:bg-text-primary/5 focus:outline-none"
			>
				<svg
					xmlns="http://www.w3.org/2000/svg"
					width="20"
...
{#if proximas.length > 0}
	<div class="mb-10">
		<div
			class="flex items-center gap-2 text-blue-600 dark:text-blue-500 mb-4 text-xs font-bold uppercase tracking-wider"
		>
			<svg
				xmlns="http://www.w3.org/2000/svg"
				width="16"
		...
		<div class="relative group">
			<button
				onclick={() => moverCarrusel(carruselProximas, 'izq')}
				class="absolute -left-5 top-1/2 -translate-y-1/2 z-10 bg-bg-card border border-border-color shadow-md rounded-full p-2 text-text-secondary opacity-0 group-hover:opacity-100 hidden sm:block hover:bg-text-primary/5 focus:outline-none"
			>
				<svg
					xmlns="http://www.w3.org/2000/svg"
			...
			<button
				onclick={() => moverCarrusel(carruselProximas, 'der')}
				class="absolute -right-5 top-1/2 -translate-y-1/2 z-10 bg-bg-card border border-border-color shadow-md rounded-full p-2 text-text-secondary opacity-0 group-hover:opacity-100 hidden sm:block hover:bg-text-primary/5 focus:outline-none"
			>
				<svg
					xmlns="http://www.w3.org/2000/svg"
...
{#if expiradas.length > 0}
	<div class="mb-10 opacity-75 hover:opacity-100 transition-opacity">
		<div
			class="flex items-center gap-2 text-text-muted mb-4 text-xs font-bold uppercase tracking-wider"
		>
			<svg
				xmlns="http://www.w3.org/2000/svg"
				width="16"
		...
		<div class="relative group">
			<button
				onclick={() => moverCarrusel(carruselExpiradas, 'izq')}
				class="absolute -left-5 top-1/2 -translate-y-1/2 z-10 bg-bg-card border border-border-color shadow-md rounded-full p-2 text-text-secondary opacity-0 group-hover:opacity-100 hidden sm:block hover:bg-text-primary/5 focus:outline-none"
			>
				<svg
					xmlns="http://www.w3.org/2000/svg"
			...
			<button
				onclick={() => moverCarrusel(carruselExpiradas, 'der')}
				class="absolute -right-5 top-1/2 -translate-y-1/2 z-10 bg-bg-card border border-border-color shadow-md rounded-full p-2 text-text-secondary opacity-0 group-hover:opacity-100 hidden sm:block hover:bg-text-primary/5 focus:outline-none"
			>
				<svg
					xmlns="http://www.w3.org/2000/svg"
		...
		>
			<header class="flex items-center justify-between border-b border-border-color p-5">
				<h2 class="text-lg font-bold text-text-primary">
					{editandoId ? 'Editar Promoción' : 'Añadir Nueva Promoción'}
				</h2>
				<button
					...
					onclick={() => (mostrarFormulario = false)}>&times;</button
				>
			</header>
			...
			<form onsubmit={handleCrearPromocion}>
				<div class="p-6 overflow-y-auto max-h-[65vh] hide-scrollbar">
					<div class="flex flex-col gap-1.5 mb-5">
						<label for="tipoPromo" class="text-[0.85rem] font-semibold text-text-secondary"
							>Tipo de Oferta:</label
						>
						<select
							id="tipoPromo"
							bind:value={tipoPromocion}
							class="rounded-lg border border-[rgba(15,30,54,0.15)] bg-white px-4 py-3 text-sm text-text-primary outline-none focus:border-primario focus:ring-1 focus:ring-primario dark:bg-bg-primary"
						>
					...
					<div class="mb-5 rounded-lg border border-border-color p-4 bg-text-primary/2">
						<label class="text-[0.85rem] font-semibold text-text-secondary mb-2 block">
							{tipoPromocion === 'COMBO'
								? 'Buscar y añadir productos (Máx 3):'
						...
						{#if tipoPromocion !== 'COMBO'}
							<select
								...
								class="w-full mb-3 rounded-lg border border-[rgba(15,30,54,0.15)] bg-white px-4 py-2 text-sm text-text-primary outline-none focus:border-primario focus:ring-1 focus:ring-primario dark:bg-bg-primary"
							>
						...
						<div
							class="flex w-full overflow-hidden rounded-lg border border-[rgba(15,30,54,0.15)] bg-white focus-within:border-primario focus-within:ring-1 focus-within:ring-primario dark:bg-bg-primary mb-3"
						>
						...
						{#if busquedaProducto.trim() && productosFiltrados.length > 0}
							<div
								class="max-h-32 overflow-y-auto rounded-lg border border-border-color bg-bg-card shadow-inner mb-3"
							>
								{#each productosFiltrados as prod}
									<button
										type="button"
										onclick={() => seleccionarProducto(prod.id_producto)}
										class="w-full text-left px-3 py-2 text-sm border-b border-border-color hover:bg-text-primary/5 text-text-primary last:border-0 flex justify-between items-center"
									>
						...
						<div class="flex flex-wrap gap-2">
							{#if tipoPromocion === 'COMBO'}
								{#each comboSeleccionados as id}
									<div
										class="flex items-center gap-2 rounded-full bg-accent/10 px-3 py-1 text-xs font-semibold text-accent border border-accent/20"
									>
										{obtenerProducto(id)?.nombre || 'Producto'}
										<button
											type="button"
											onclick={() => removerProductoCombo(id)}
											class="text-accent hover:text-danger-color">&times;</button
										>
									</div>
								{/each}
					...
					<div class="grid grid-cols-1 sm:grid-cols-2 gap-5 mb-5">
						<div class="flex flex-col gap-1.5">
							<label for="fechaInicio" class="text-[0.85rem] font-semibold text-text-secondary"
						...
						<div class="flex flex-col gap-1.5">
							<label for="fechaFin" class="text-[0.85rem] font-semibold text-text-secondary"
								>Fecha de Fin:</label
							>
							<input
								type="date"
					...
					<div class="bg-text-primary/5 p-4 rounded-lg border border-border-color">
						{#if tipoPromocion === 'NXM'}
							<div class="flex gap-4">
								<div class="flex-1 flex flex-col gap-1.5">
									<label for="inputLleva" class="text-[0.85rem] font-semibold text-text-secondary"
										>Lleva:</label
									>
									<input
										id="inputLleva"
								...
								<div class="flex-1 flex flex-col gap-1.5">
									<label for="inputPaga" class="text-[0.85rem] font-semibold text-text-secondary"
										>Paga:</label
									>
									<input
										id="inputPaga"
						...
						{:else if tipoPromocion === 'precio_fijo' || tipoPromocion === 'COMBO'}
							<div class="flex flex-col gap-1.5">
								<label for="inputDesc" class="text-[0.85rem] font-semibold text-text-secondary"
									>{tipoPromocion === 'COMBO'
										? 'Precio Final del Combo ($):'
										: 'Monto a descontar ($):'}</label
								>
								<input
									id="inputDesc"
									type="number"
						...
						{:else if tipoPromocion === 'porcentaje'}
							<div class="flex flex-col gap-1.5">
								<label for="inputDescPorc" class="text-[0.85rem] font-semibold text-text-secondary"
				...
				>
					<button
						type="button"
						class="inline-flex cursor-pointer items-center justify-center gap-2 rounded-lg border border-border-color bg-bg-secondary px-5 py-2.5 text-sm font-semibold text-text-primaryl hover:bg-text-primary/5"
						onclick={() => (mostrarFormulario = false)}
						disabled={submitLoading}>Cancelar</button
					>
					<button
						type="submit"
						class="inline-flex cursor-pointer items-center justify-center gap-2 rounded-lg bg-gradient-to-r from-accent-light to-accent px-5 py-2.5 text-sm font-semibold text-white hover:-translate-y-[1px] hover:shadow-glow disabled:cursor-not-allowed disabled:opacity-50"
	...
	>
		...
		>
			<header class="flex items-center justify-between border-b border-border-color p-5">
				...
				<button
					class="inline-flex cursor-pointer items-center justify-center rounded-lg border border-border-color bg-text-primary/3 p-2 text-text-secondary hover:border-border-color-hover hover:bg-text-primary/7 hover:text-text-primary"
					onclick={() => (mostrarModalEliminar = false)}>&times;</button
				>
			</header>
			<div class="p-6 text-text-primary">
				<p>¿Estás seguro de que deseas cancelar esta promoción de forma permanente?</p>
				<p class="mt-3 text-xs text-text-muted">
					La promoción pasará a estar inactiva y los clientes ya no podrán usar este descuento.
				</p>
			</div>
			<footer
				class="flex justify-end gap-3 border-t border-border-color bg-text-primary/2 p-4 px-6"
			>
				<button
					...
					onclick={() => (mostrarModalEliminar = false)}>Mantener promoción</button
				>
				<button
					type="button"
					class="inline-flex cursor-pointer items-center justify-center gap-2 rounded-lg border border-red-500/15 bg-danger-bg px-5 py-2.5 text-sm font-semibold text-danger-color hover:bg-danger-color hover:text-white"
					onclick={confirmarEliminacion}>Sí, cancelar ahora</button
				>
			</footer>
		</div>
	</div>
