...
export const apiClientes = {
	...
	searchByNombre: (nombre: string) =>
		apiFetch(`/clientes/search/nombre?nombre=${encodeURIComponent(nombre)}`),
	create: (data: { nombre: string; rut: string; telefono: string }) =>
		apiFetch('/clientes', {
			method: 'POST',
			body: JSON.stringify(data)
		}),
	update: (id: string, data: { nombre?: string; rut?: string; telefono?: string }) =>
...
export const apiCategorias = {
	...
	create: (data: { nombre_categoria: string }) =>
		apiFetch('/inventario/categorias', {
			method: 'POST',
			body: JSON.stringify(data)
		}),
	update: (id: string, data: { nombre_categoria: string }) =>
		apiFetch(`/inventario/categorias/${id}`, {
			method: 'PUT',
			body: JSON.stringify(data)
		}),
	delete: (id: string) =>
...
export const apiProductos = {
	...
	update: (id: string, data: Partial<Producto>) =>
		apiFetch(`/inventario/productos/${id}`, {
			method: 'PUT',
			body: JSON.stringify(data)
		}),
	delete: (id: string) =>
...
export const apiRoles = {
	...
	create: (data: any) =>
		apiFetch('/empleados/roles', { method: 'POST', body: JSON.stringify(data) }),
	update: (id: string, data: any) =>
		apiFetch(`/empleados/roles/${id}`, { method: 'PATCH', body: JSON.stringify(data) }),
...
export const apiCajas = {
	...
	update: (id: string, data: any) =>
		apiFetch(`/cajas/${id}`, { method: 'PATCH', body: JSON.stringify(data) }),
...
export const apiTurnos = {
	getActivo: (): Promise<{ activo: boolean; turno?: TurnoCaja }> =>
		apiFetch('/cajas/turnos/activo'),
